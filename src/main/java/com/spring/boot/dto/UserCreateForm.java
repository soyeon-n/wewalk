...
//	@Size(min = 6, max = 16, message = "닉네임은 6~16자 이내로 생성해주세요!")
	...
	/*
	 * @Pattern(regexp =
	 * "^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[@#$%^&+=])(?=\\S+$).{6,}$", message
	 * = "비밀번호는 최소 6자 이상이어야 하며, 영문자 대소문자와 특수문자를 모두 포함해야 합니다.")
	 */
...
//	@Size(min = 10, max = 11, message = "전화번호는 10자리 또는 11자리여야 합니다.")
	...
	/*
	 * @Pattern(regexp = "^(0?[1-9]|1[012])$", message =
	 * "태어난 월은 1부터 12 사이의 숫자만 입력 가능합니다.")
	 */
	private String birthMonth;
	
	@NotEmpty(message = "태어난 일은 필수 항목입니다.")
	/*
	 * @Pattern(regexp = "^(0?[1-9]|1[0-9]|2[0-9]|3[0-1])$", message =
	 * "태어난 일은 숫자만 입력 가능합니다.")
	 */
